...
def train(args, train_dataset,eval_dataset, model: PreTrainedModel, tokenizer: PreTrainedTokenizer) -> Tuple[int, float]:
    ...
    def collate(examples: List[torch.Tensor]):
        return torch.stack([exam['input_ids'] for exam in examples])

        if tokenizer._pad_token is None:
            return pad_sequence(examples, batch_first=True)
        # return pad_sequence(examples, batch_first=True, padding_value=tokenizer.pad_token_id)
        # return pad_sequence(examples['input_ids'], batch_first=True, padding_value=tokenizer.pad_token_id)
        return pad_sequence([exam['input_ids'] for exam in examples], batch_first=True,
...
def main():
    ...
    # Required parameters
    parser.add_argument(
        "--train_data_file", default='/data/models/datasets/alclam/plm/train.json', type=str, help="The input training data file (a text file)."
    )

    parser.add_argument(
        "--output_dir",
        type=str,
        default='models_alclam/',
    ...
    parser.add_argument(
        "--baseline", type=str, default='alclam', help="The model architecture to be trained or fine-tuned.",
    )

    # Other parameters
    parser.add_argument(
        "--eval_data_file",
        default='/data/models/datasets/alclam/plm/test.json',
        type=str,
    ...
    if args.do_train:
        ...
        dataset = load_dataset('json', data_files={"train": args.train_data_file, "test": args.eval_data_file})

        def encode(example):

            # return tokenizer(example['text'], return_tensors='pt', add_special_tokens=True, max_length=512, truncation=True, padding=True)
            return tokenizer(example['text'], return_tensors='pt', add_special_tokens=True, max_length=128, truncation=True, padding=True)
            # return tokenizer(example['text'], return_tensors='pt', add_special_tokens=True, max_length=tokenizer.model_max_length,truncation=True, padding=True)
        ...
        dataset = dataset.filter(filter_min_tokens)
